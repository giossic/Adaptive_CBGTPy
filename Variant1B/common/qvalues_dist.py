    ...
    Q_support_params = ParamSet('Q_support_params',
                                {
                                 ...
                                 #'dpmn_CPP_scale': 35.,
                                 'C_scale': 80/1000, #80 #90/1000, 110/1000
                                 #'dpmn_CPP_scale': 25.,
                                 'reward_value': -1.,
                                 'chosen_action': 1})
...
def helper_update_Q_df(Q_df, Q_support_params, dpmndefaults, trial_num, current_RT, target_RT):

    # Required to perform mathematical calculations with data frame values
    Q_support_params = untrace(Q_support_params)
    ppf = sp_st.norm(loc=target_RT, scale=0.2)
    ...
    if Q_support_params.chosen_action[0] != 'stop' and Q_support_params.chosen_action[0] != 'none':
        ...
        print('current RT', current_RT)
        print('target RT', target_RT)
        #print('current q-value', q_val_chosen)
        #q_error =  Q_support_params.reward_value.values*np.exp(-alpha_RT*(current_RT-target_RT)**2)
        ...
        in_dist = ppf.cdf(target_RT) -  ppf.cdf(current_RT)
        if in_dist < 0.5: 
            da_inc = 0
        else: 
            da_inc =  -(target_RT - current_RT)
        ...
        factor = np.abs(target_RT - current_RT)/(target_RT + current_RT)
