...
#                 agent.stoptimer_2 = 0
            ...
            if self.stop_signal_present[i]:
                if isinstance(stop_duration[i], (float,int)): 
                    ...
                    if self.trial_num in self.stop_list_trials_list[i]:
                        ...
                        if not printed_rt_this_trial and current_rt_decay > 0:
                            if 'current_rt_decay' in locals():
                                print("current_rt_decay trial", self.trial_num, ":", current_rt_decay)
                            else:
                                pass
                        ...
                        if agent.motor_queued == -1 or datatables_decision=='stop': 
                            ...
                            # -------------------------
                            # FAILED STOP → START / CONTINUE DECAY
                            # -------------------------
    ...
    #                         print(which_phase_df)
                            ...
                            if agent.phase != int(which_phase[1]): # Stop stimulation if the phase is over
                                ...
                                # agent.FreqExt_AMPA[popid] = agent.FreqExt_AMPA_basestim[popid] + np.success(zero or ones) * np.exp((datatables_decisiontime-agent.globaltimer)/0.02) * (agent.FreqExt_AMPA[popid] - agent.FreqExt_AMPA_basestim[popid])
                                 # 1) Solo decaimiento por CS y FS 
                                
                
                                #if self.chosen_action is not None: 
                                     #if self.chosen_action == 'left': FAILED STOP 
                                         #agent.FreqExt_AMPA[popid] = (agent.FreqExt_AMPA[popid] - agent.FreqExt_AMPA_basestim[popid]) * np.exp((current_rt-agent.globaltimer)/2) + agent.FreqExt_AMPA_basestim[popid]
                                     #else: CORRECT STOP
                                          #agent.FreqExt_AMPA[popid] = agent.FreqExt_AMPA_basestim[popid]
