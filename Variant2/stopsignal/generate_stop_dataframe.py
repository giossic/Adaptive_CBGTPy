...
def define_stop(stop_signal_probability, actionchannels, n_trials, pop_names, stop_signal_channel, stop_signal_amplitude, stop_signal_onset, stop_signal_duration, stop_signal_present, stop_signal_population, stop_signal_onset_type):
    ...
    for i in np.arange(len(stop_signal_population)):
        ...
        for act in list(actionchannels.action.values):
            #if len(stop_signal_onset[i]) > 1:
            stop_onset_df[act] = False
            #else:
                #stop_onset_df[act] = stop_signal_onset[i]
        
        #print('stop_signal_onset[i] generate stop dataframe', stop_signal_onset[i])
    ...
    #     print(pops)
        ...
        stop_onset_dfs.append(stop_onset_df)
...
def GenStopSchedule(stop_signal_probability, actionchannels, n_trials, popdata, stop_signal_channel, stop_signal_amplitude, stop_signal_onset, stop_signal_duration, stop_signal_present, stop_signal_population, stop_signal_onset_type):
    
    stop_df, stop_channels_dfs, stop_amplitude_dfs, stop_onset_dfs, stop_duration_dfs, stop_populations_dfs, stop_list_trials_list = define_stop(stop_signal_probability, actionchannels, n_trials, popdata, stop_signal_channel, stop_signal_amplitude, stop_signal_onset, stop_signal_duration, stop_signal_present, stop_signal_population, stop_signal_onset_type)
