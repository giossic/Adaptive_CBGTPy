...
def multitimestep_mutator(agent,popdata,numsteps):
    #print(np.random.randint(0,999999999,10))
    for i in range(numsteps):
        timestep_mutator(agent,popdata)

def get_fDA(DA,dpmn_type,dpmn_x_fda,dpmn_y_fda, dpmn_d_fda, dpmn_d2_DA_eps):
    #print("DA_shape",np.shape(DA))
    #print("DA", DA)
    #print("dpmn_x_fda_shape",np.shape(dpmn_x_fda))
    #print("dpmn_x_fda",dpmn_x_fda)
    
    fda = np.zeros(len(DA))
    if dpmn_type < 1.5: # D1
        
        fda = (
            (-dpmn_y_fda) * (DA < -dpmn_x_fda) + 
            ((dpmn_y_fda/dpmn_x_fda)*DA) * ((-dpmn_x_fda <= DA) & (DA < 0*dpmn_x_fda)) +
            (dpmn_d_fda*DA) * (DA >= 0*dpmn_x_fda) )
...
def timestep_mutator(a,popdata):
        ...
        for dest_popid in range(len(popdata)):
            if a.AMPA_con[src_popid][dest_popid] is not None:
                for src_neuron in a.spikes[src_popid]:
                    ...
                    elif a.dpmn_cortex_stop[src_popid][0] > 0: 
                        #print('inside 1')
                        a.dpmn_XPRE[dest_popid] = np.maximum(a.dpmn_XPRE[dest_popid], a.dpmn_cortex_stop[src_popid][src_neuron] * a.AMPA_con[src_popid][dest_popid][src_neuron] * np.sign(a.dpmn_type[dest_popid]))
                    else:
                        a.dpmn_XPRE[dest_popid] = np.maximum(a.dpmn_XPRE[dest_popid], a.dpmn_cortex[src_popid][src_neuron] * a.AMPA_con[src_popid][dest_popid][src_neuron] * np.sign(a.dpmn_type[dest_popid]))
    ...
    for popid in range(len(popdata)):
        ...
        a.ExtSigmaS_NMDA[popid] = a.MeanExtEff_NMDA[popid] * np.sqrt(a.Tau_NMDA[popid] * .5 * a.FreqExt_NMDA[popid] * .0005 * a.MeanExtCon_NMDA[popid]) #.001
    ...
    for popid in range(len(popdata)):
        if a.dpmn_type[popid][0] > 0:
            ...
            if a.dpmn_type[popid][0] > 1.5:
                ...
                a.dpmn_E_stop[popid] += a.dt * (a.dpmn_XPOST[popid] * a.dpmn_APRE[popid] - a.dpmn_XPRE[popid] * a.dpmn_APOST[popid] - a.dpmn_E[popid]) / a.dpmn_tauE[popid]


                #a.dpmn_E_stop[popid] += a.dt * (a.dpmn_APOST[popid] - a.dpmn_E[popid]) / 30.
                #a.dpmn_XPRE[popid] * 
                
            DA = a.dpmn_m[popid] * (a.dpmn_DAp[popid] + a.dpmn_DAt[popid])
            
            if a.dpmn_type[popid][0] < 1.5:
                fDA = a.dpmn_fDA_D1 = get_fDA(DA,a.dpmn_type[popid][0],a.dpmn_x_fda[popid],a.dpmn_y_fda[popid],a.dpmn_d_fda[popid], a.dpmn_d2_DA_eps[popid])
            elif a.dpmn_type[popid][0] > 1.5:
                fDA = a.dpmn_fDA_D2 = get_fDA(DA,a.dpmn_type[popid][0],a.dpmn_x_fda[popid],a.dpmn_y_fda[popid],a.dpmn_d_fda[popid], a.dpmn_d2_DA_eps[popid])
            #print("fDA",fDA)
            ...
            for src_popid in range(len(popdata)):
                ...
                if a.dpmn_cortex[src_popid][0] > 0:
                    ...
                    if a.AMPA_con[src_popid][popid] is not None:
                        ...
                        #print(ind_neg)
                        #a.AMPA_eff[src_popid][popid] += (update * ind_neg * (a.AMPA_eff[src_popid][popid] - 0.001)) #w_min=0.001 original value
                        a.AMPA_eff[src_popid][popid] += (update * ind_neg * (a.AMPA_eff[src_popid][popid] - 0.010)) #0.01
                    ...
                    if a.AMPA_con[src_popid][popid] is not None:
                        ...
                        update = a.dt * a.AMPA_con[src_popid][popid] * a.dpmn_alphaw_stop[popid] * fDA #* (0.002 - a.dpmn_E_stop[popid])
                        
                        update = np.maximum(update,-1)

                        update = np.minimum(update,1)
                        ind_pos = np.greater(update,0).astype(int)
                        #a.ind_pos = (update * ind_pos * (a.dpmn_wmax[popid] - a.AMPA_eff[src_popid][popid])) 

                        a.AMPA_eff[src_popid][popid] += (update * ind_pos * (a.dpmn_wmax_stop[popid] - a.AMPA_eff[src_popid][popid])) 
                       
                        ind_neg = np.less(update,0).astype(int)
                        #a.ind_neg= (update * ind_neg * (a.AMPA_eff[src_popid][popid] - 0.010))
                        
                        a.AMPA_eff[src_popid][popid] += (update * ind_neg * (a.AMPA_eff[src_popid][popid] - 0.010))
