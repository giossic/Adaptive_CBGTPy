...
def plot_fr(firing_rates, datatables, results, experiment_choice,display_stim=False):
    ...
    if experiment_choice == "n-choice":
        ...
        col_order = ["Cx", "CxS", "CxI", "FSI","GPeP", "GPeA", "iSPN", "dSPN", "STN","GPi","Th"]
        aspect = 1.4
                ...
                if results[i]['stop_signal_present'][ni] == True:
                    ...
                    for tr in results[i]['stop_list_trials_list'][ni]:
                        if isinstance(results[i]['stop_duration_dfs'][ni].iloc[0][0],str):
                            ...
                            start = datatables[i].stimulusstarttime[tr]+results[i]['stop_signal_onset'][ni][0]
                            end = datatables[i].stimulusstarttime[tr]+results[i]['stop_signal_onset'][ni][0]+results[i]['stop_signal_duration'][ni]
                            for ax in axes:
...
def plot_fr_new(firing_rates, datatables, results, experiment_choice,display_stim=False):
            ...
            for ni,nuc in enumerate(results[i]['stop_signal_population']):
                ...
                if results[i]['stop_signal_present'][ni] == True:
                    ...
                    for tr in results[i]['stop_list_trials_list'][ni]:
                        ...
                        elif isinstance(datatables[i].stopduration[tr],(float,int)):
                            start = datatables[i].stimulusstarttime[tr]+datatables[i].stoponset[tr]
                            end = datatables[i].stimulusstarttime[tr]+datatables[i].stoponset[tr]+datatables[i].stopduration[tr]
                            for ax in axes:
                                g1.axes[ind].hlines(y=ylim[1],xmin= start,xmax=end, 
                                                    color='red', linewidth=15)
