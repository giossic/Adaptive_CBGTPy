...
def extract_relevant_frames(results,seed,experiment_choice):
    ...
    err_df = cbgt.collateVariable(results,'err_df')
    ...
    #         perf["Q_val->dopamine_scale"] = [ dpmn_cpp_scale for j in np.arange(len(perf))]
        ...
        for i in np.arange(len(results)):
            ...
            chosen_action = pd.DataFrame(datatables[i]["decision"].copy())
            chosen_action["trials"] = chosen_action.index
            chosen_action = chosen_action.reset_index(drop=True)

            ttype = pd.DataFrame(datatables[i]["ttype"].copy())
            
            Q_df_local = Q_df[i].iloc[1:].copy()
            Q_df_local = Q_df_local.rename(columns={"left":"qvalues"})
            Q_df_local = Q_df_local.rename(columns={"da_inc_scaled":"da_inc_go"})
            Q_df_local = Q_df_local.drop('reward_adj', axis=1)
            Q_df_local = Q_df_local.drop('da_inc', axis=1)
            Q_df_local = Q_df_local.drop('target_rt', axis=1)
            
            err_df_local = pd.DataFrame(err_df[i].iloc[1:][['da_inc_scaled', 'left']].copy())
            err_df_local = err_df_local.rename(columns={"da_inc_scaled":"da_inc_stop"})
            err_df_local = err_df_local.rename(columns={"left":"error_exp_stop"})
            
            rew_df = datatables[i]['reward'].copy()
            rew_df = rew_df.reset_index(drop=True)

            rt_df = datatables[i]['decisionduration'].copy()
            #rt_df = rt_df.rename({"decisionduration":"current_rt"})
            rt_df = rt_df.reset_index(drop=True)
           
            Q_df_local = Q_df_local.reset_index(drop=True)
            err_df_local = err_df_local.reset_index(drop=True)

            target_rt_df = Q_df[i].iloc[1:]['target_rt'].copy()
            target_rt_df = target_rt_df.reset_index(drop=True)
            
            
            final_data = pd.concat([chosen_action, ttype], axis=1)
            final_data = pd.concat([final_data,rt_df], axis=1)#Q_df_local.append(rew_df)
            final_data = pd.concat([final_data,rew_df], axis=1)#Q_df_local.append(rew_df)
            final_data = pd.concat([final_data, Q_df_local], axis=1)#final_data.append(chosen_action)
            final_data = pd.concat([final_data, err_df_local], axis=1)
            final_data = pd.concat([final_data, target_rt_df], axis=1)
            
            final_data["seed"] = [str(seed)+"_"+str(i)  for j in np.arange(len(final_data))]
            
            final_data = final_data.reset_index(drop=True)
            final_data = final_data[['trials', 'decisionduration', 'decision', 'reward', 'ttype', 'qvalues', 'q_error_RPE', 'da_inc_go', 'da_inc_stop', 'error_exp_stop', 'target_rt', 'seed']]

            q_err_df.append(final_data)

        return firing_rates, rt_distribution, q_err_df
