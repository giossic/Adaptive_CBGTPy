...
def get_reward_value_edit(chosen_action, trial_num, current_RT, target_RT, scale_reward, stop_list_trials_list):
    ...
    if trial_num not in stop_list_trials_list: #if ttype == 'go':
        ...
        if chosen_action == 'none': 
            ...
            reward_val = 100 - scale_reward*(np.abs(target_RT - current_RT))
    else: 
...
def helper_update_Q_df(Q_df, Q_support_params, dpmndefaults, trial_num, current_RT, target_RT, timeout_RT, scale_power, scale_factor_stop, max_stop_amplitude, stop_list_trials_list, stop_amp, stop_iter):
    ...
    if trial_num not in stop_list_trials_list: #if ttype == 'go':
        ...
        if Q_support_params.chosen_action[0] != 'none':
            ...
            #print('q_val_chosen', q_val_chosen)
            
            #q_error = (target_RT - current_RT)
            q_error = Q_support_params.reward_value.values - q_val_chosen.values # Normalize so that 100:1
            
            da_inc =  -np.sign(target_RT - current_RT)*((np.abs(target_RT - current_RT)/(target_RT + current_RT))**scale_power)  #np.real()
            #print('da_inc - go:', da_inc)
           
            #q_val_updated = current_RT + Q_support_params.q_alpha.values * q_error 
            q_val_updated = (q_val_chosen.values + Q_support_params.q_alpha.values * q_error)/100
            ...
            dpmndefaults.dpmn_DAp = da_inc * Q_support_params.C_scale #* factor**1.5
            ...
            Q_df = pd.concat([Q_df,new_data])
            da_inc =  -np.sign(target_RT - current_RT)*((np.abs(target_RT - current_RT)/(target_RT + current_RT))**scale_power)
        ...
        if Q_support_params.chosen_action[0] == 'stop':
            ...
            Q_df = pd.concat([Q_df,new_data])
            da_inc =  -np.sign(timeout_RT - current_RT)*((np.abs(timeout_RT - current_RT)/(timeout_RT+current_RT))**scale_power)
            ...
            #print('newQdf', Q_df)
        #print('scale_power', scale_power)

    return Q_df, Q_support_params, dpmndefaults, stop_amplitude_q
